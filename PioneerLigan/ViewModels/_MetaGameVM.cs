...
namespace PioneerLigan.ViewModels
{
    public class _MetaGameVM
    {
        ...
        {
            var decks = JsonConvert.DeserializeObject<List<DeckInfo>>(meta);
            var totalNoOfDecks = decks.Sum(d => d.NoOfDecks);

            foreach (var deck in decks)
            {
                Decks.Add(new MetaDeck { Name = deck.DeckName, Count = deck.NoOfDecks, Percentage = (int)Math.Round((double)deck.NoOfDecks / totalNoOfDecks * 100) });
            }
